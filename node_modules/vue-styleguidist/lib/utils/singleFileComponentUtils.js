...
var buildStyles = function buildStyles(styles) {
  ...
  if (_styles !== '') {
    return "<style scoped>".concat(_styles.trim(), "</style>");
  }
...
var getSingleFileComponentParts = function getSingleFileComponentParts(code) {
  var parts = compiler.parseComponent(code, {
    pad: 'line'
  });
  parts.script.content = stripComments(parts.script.content, {
...
var injectTemplateAndParseExport = function injectTemplateAndParseExport(parts) {
  ...
  var templateString = replaceAll("".concat(parts.template.content), '`', '\\`');
  return "{\ntemplate: `".concat(templateString, "`,\n").concat(right);
};
...
export function transformSingleFileComponent(code) {
  ...
  return {
    component: "\n\t\t\t".concat(extractImports(parts.script.content), "\n\t\t\tnew Vue(").concat(templateAdded, ");\n\t\t"),
